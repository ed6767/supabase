...
export default function TicketContainer({ user, sharePage, referrals, supabase }: Props) {
  ...
  return (
    <>
          ...
          <div className="text-scale-1200 flex flex-col w-full items-center text-white text-center lg:text-left lg:items-start gap-2 lg:gap-3">
            <h1
              className={cn(
                // styleUtils.appear,
                styleUtils['appear-first'],
                'text-2xl tracking-[-0.02rem] leading-7 block text-white'
              )}
            >
              {!sharePage ? (
                name ? (
                ...
                golden ? (
                  ...
                  <p>
                    ...
                    <Link href="#lw8-prizes">
                      <a className="hover:!underline !text-brand-900">limited edition awards</a>
                    </Link>{' '}
      ...
      {/* {sharePage && (
        <TicketDisclaimer className="mt-4 md:mt-8 relative z-40">
          Want to delete your ticket?{' '}
          <a href="mailto:support@supabase.com" className="hover:underline">
            Submit a request
          </a>
          .
        </TicketDisclaimer>
